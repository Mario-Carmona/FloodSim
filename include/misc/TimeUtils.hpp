...
/** @brief A time point with double precision for internal engine use. */
using sys_time_double = std::chrono::time_point<std::chrono::system_clock, std::chrono::duration<double>>;
...
/** * @brief Parses a duration string formatted as HH:MM:SS.sss into fractional chrono seconds.
 * * @param duration_str The input duration string (e.g., "00:00:00.25").
 * @return std::chrono::duration<double> Total duration measured in seconds with floating-point precision.
 * @throw floodsim::app::exception::FloodSimException If syntax constraints or colon separators are missing.
 */
[[nodiscard]] inline std::chrono::duration<double> ParseFractionalDurationString(const std::string& duration_str) {
    std::istringstream ss{ duration_str };
    int hours = 0;
    int minutes = 0;
    double seconds = 0.0;
    ...
    if (ss.fail() || colon1 != ':' || colon2 != ':') {
        throw floodsim::app::exception::FloodSimException("Failed to parse fractional duration string: " + duration_str);
    }
