...
for a clear decoupling between the model logic and the implementation, facilitating extensibility. Recently, this framework 
has been fully generalized into the multi-n-dimensional cellular automaton ($m:n-CA^k$), extending its capabilities 
to operate in tensorial spaces and support continuous space evolution alongside irregular grids, which simplifies the 
integration of heterogeneous geospatial datasets \cite{FonsecaiCasas2025}.
...
Despite advancements in discrete simulation, catastrophic flash floods—such as the 2024 DANA in the Júcar River Basin—have exposed critical 
vulnerabilities in current modeling frameworks. These deficiencies can be categorized into four interconnected areas: high computational 
latency, architectural rigidity, oversimplified spatial representations, and a pronounced socio-technical gap in emergency management.

\subsection{Computational Latency vs. Flash Flood Dynamics}

The execution time of physically-based Shallow Water Equation (SWE) solvers remains a primary constraint. Achieving high-precision results 
on complex meshes requires high-performance computing (HPC) resources that are too slow to deploy during the narrow lead-time of a flash 
flood. When peak flows occur within minutes, the computational overhead of traditional numerical methods limits their utility to post-event 
forensic analysis rather than proactive forecasting \parencite{Muoz2025}.

\subsection{Monolithic Architectural Rigidity}

Many flood simulation platforms rely on monolithic architectures that lack modularity, making it difficult to incorporate new infiltration 
algorithms or dynamic weather inputs without comprehensive system overhauls. This rigidity severely restricts interoperability with diverse 
GIS formats and hinders alignment with modern, adaptive frameworks like the Society 5.0 paradigm \parencite{FonsecaiCasas2026}.

\subsection{Simplified Spatial Heterogeneity}

While standard Cellular Automata (CA) offer the execution speeds required for real-time applications, they frequently oversimplify the 
physical environment. Most CA implementations fail to capture complex, multi-layered spatial data—such as dynamic soil saturation, 
subterranean drainage, and varying urban roughness coefficients—within a unified tensor structure, limiting their capacity to simulate 
non-linear urban hydraulics \parencite{Fonseca2011}.

\subsection{The Socio-Technical Gap in Emergency Management}

A critical disconnect persists between the technical outputs of simulation models and the operational needs of first responders. As 
observed during the 2024 DANA crisis, the lack of an operational, real-time "Digital Twin" meant that decision-makers operated with 
fragmented and delayed information. Existing systems fail to natively ingest modern IoT and radar streams necessary to provide a 
continuously synchronized representation of an evolving disaster.
