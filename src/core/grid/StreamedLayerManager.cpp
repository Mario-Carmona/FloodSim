...
namespace danasim {
    ...
    void StreamedLayerManager::initLayer(MapGrid& grid, const std::filesystem::path& filePath, float timeStep, LayerId layerId) {
        try {
    ...
    void StreamedLayerManager::initRainfallLayer(HighFive::File& file, LayerId layerId) {
        // 1. Access the "Rainfall" Group (depending on your Python)
        auto group = file.getGroup("Rainfall");

        // 2. Access the "Data" dataset within Rainfall
        dataset_[layerId] = HighFive::DataSet(group.getDataSet("Data"));

        // 3. Read Group Attributes
        group.getAttribute("Downgrade_Factor").read(downgradeFactor_[layerId]);
        group.getAttribute("Time_Step_Seconds").read(h5TimeStepSeconds_[layerId]);
    }

    void StreamedLayerManager::initLayerBuffer(const MapGrid& grid, LayerId layerId) {
    ...
    void StreamedLayerManager::updateAllLayers(MapGrid& grid, StepType currentStep, float timeStep) {
        ...
        for (auto& value : magic_enum::enum_values<LayerId>()) {
            ...
            if (desc.source == LayerSource::Streamed) {
    ...
    void StreamedLayerManager::updateLayer(MapGrid& grid, size_t frameIdx, float timeStep, LayerId layerId) {
        try {
    ...
    float StreamedLayerManager::calculateLayerValue(float bufferValue, float timeStep, LayerId layerId) {
        switch (layerId) {
