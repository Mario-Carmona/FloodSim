...
package Userinterface_RI is

   procedure Loadmaprequest (Folder : in out asn1SccFolderpath; Dest_PID : asn1sccPID := asn1sccEnv);
   
   
   procedure Simulationstart (Dest_PID : asn1SccPID := asn1sccEnv);

   procedure Check_Queue (Res : out adaasn1rtl.Asn1Boolean)
      with Import, Convention => C, Link_Name => "userinterface_check_queue";

   procedure Get_Sender (Sender : out asn1sccPID; Dest_PID : asn1SccPID := asn1sccEnv)
      with Import, Convention => C, Link_Name => "userinterface_get_sender";

   procedure Get_Last_Error (Err : out asn1sccT_Runtime_Error; Dest_PID : asn1SccPID := asn1sccEnv)
      with Import, Convention => C, Link_Name => "userinterface_get_last_error";
